...
#
# Tracks rev is now of the form MAJOR.MINOR.MICRO-rev-commit
# or, if right at the same rev as a release, MAJOR.MINOR.MICRO
#
...
int main() { return 0; }''',
    ...
    if conf.options.cxx11 or conf.env['build_host'] in [ 'mavericks', 'yosemite' ]:
        if not conf.is_tracks_build(): 
            conf.check_cxx(cxxflags=["-std=c++11"])
            cxx_flags.append('-std=c++11')
        if platform == "darwin":
            # Set libstdc++ flag to use std lib implementation 
...
def configure(conf):
    ...
    conf.env['EXECUTABLE_NAME'] = 'trackslive'
...
''',
    ...
    if conf.is_tracks_build():
        ...
        if sys.platform == 'darwin' or Options.options.dist_target == 'mingw' or sys.platform == 'msvc':
            backends = [ 'wavesaudio' ]
