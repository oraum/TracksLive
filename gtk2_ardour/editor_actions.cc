...
Editor::register_actions ()
{
	...
	act = ActionManager::register_action (editor_menu_actions, X_("MovePlayHeadMenu"), _("Playhead"));
    ActionManager::record_and_playback_restricted_actions.push_back (act);
    act = ActionManager::register_action (editor_menu_actions, X_("PlayHeadModes"), _("Playhead Modes"));
    ActionManager::record_and_playback_restricted_actions.push_back (act);
	...
	act = ActionManager::register_action (editor_menu_actions, X_("SelectMenu"), _("Select"));
    ActionManager::record_and_playback_restricted_actions.push_back (act);
	...
	*/
	...
	act = reg_sens (editor_actions, "select-all", _("Select All"), sigc::mem_fun(*this, &Editor::select_all_regions ));
    ActionManager::record_and_playback_restricted_actions.push_back (act);
    ActionManager::region_exists_sensitive_actions.push_back (act);
	act = reg_sens (editor_actions, "invert-selection", _("Invert Selection"), sigc::mem_fun(*this, &Editor::invert_selection ));
    ActionManager::record_and_playback_restricted_actions.push_back (act);
    ActionManager::region_or_time_selection_sensitive_actions.push_back (act);
    act = toggle_reg_sens (editor_actions, "toggle-enable-group-edit", _("Enable Group Edit"), sigc::mem_fun (*this, &Editor::toggle_enable_group_edit));
    ActionManager::record_and_playback_restricted_actions.push_back (act);
	...
	act = reg_sens (editor_actions, "jump-forward-to-mark", _("Jump to Next Marker"), sigc::mem_fun(*this, &Editor::jump_forward_to_mark));
    
	act = reg_sens (editor_actions, "alternate-jump-forward-to-mark", _("Jump to Next Marker"), sigc::mem_fun(*this, &Editor::jump_forward_to_mark));
    ActionManager::record_and_playback_restricted_actions.push_back (act);
    
	act = reg_sens (editor_actions, "jump-backward-to-mark", _("Jump to Previous Marker"), sigc::mem_fun(*this, &Editor::jump_backward_to_mark));
    
    act = reg_sens (editor_actions, "alternate-jump-backward-to-mark", _("Jump to Previous Mark"), sigc::mem_fun(*this, &Editor::jump_backward_to_mark));
    ActionManager::record_and_playback_restricted_actions.push_back (act);
	...
	ActionManager::record_and_playback_restricted_actions.push_back (undo_action);

	redo_action = reg_sens (editor_actions, "redo", _("Redo"), sigc::bind (sigc::mem_fun(*this, &Editor::redo), 1U));
	ActionManager::record_and_playback_restricted_actions.push_back (redo_action);
	...
	act = reg_sens (editor_actions, "alternate-editor-delete", _("Delete"), sigc::mem_fun(*this, &Editor::delete_));
    ActionManager::record_and_playback_restricted_actions.push_back (act);
    ActionManager::region_list_selection_sensitive_actions.push_back (act);
...
//	ActionManager::register_action (editor_actions, "set-edit-splice", _("Splice"), sigc::bind (sigc::mem_fun (*this, &Editor::set_edit_mode), Splice));
	...
	ruler_marker_action = Glib::RefPtr<ToggleAction>::cast_static (ActionManager::register_toggle_action (ruler_actions, X_("toggle-marker-ruler"), _("Marker Ruler"), sigc::bind (sigc::mem_fun(*this, &Editor::toggle_ruler_visibility), ruler_time_marker)));
	ruler_marker_action->connect_proxy (get_check_menu_item ("global_tracks_marker_ruler_menu_item"));
	...
	ruler_skip_action = Glib::RefPtr<ToggleAction>::cast_static (ActionManager::register_toggle_action (ruler_actions, X_("toggle-skip-ruler"), _("Skip Ruler"), sigc::bind (sigc::mem_fun(*this, &Editor::toggle_ruler_visibility), ruler_time_skip)));
	ruler_skip_action->connect_proxy (get_check_menu_item ("global_tracks_skip_ruler_menu_item"));
...
Editor::parameter_changed (std::string p)
{
	...
	if (p == "auto-loop") {
        ...
        if (_session) {
            double amlitude_zoom = _session->config.get_wave_amplitude_zoom ();
...
Editor::register_region_actions ()
{
    ...
    /* Move selected regions to the current position of playhead*/
    reg_sens (_region_actions, "regions-to-playhead", _("Move to Playhead Position"), sigc::mem_fun (*this, &Editor::regions_to_playhead));

    /* Toggle `locked' status of selected regions */
	toggle_reg_sens (_region_actions, "toggle-region-lock", _("Lock Position"), sigc::mem_fun(*this, &Editor::toggle_region_lock));
    ...
    reg_sens (_region_actions, "mute-region", _("Mute Region"), sigc::mem_fun(*this, &Editor::toggle_region_mute));
    ...
    reg_sens (_region_actions, "region-to-session-start", _("Align to Session Start"), sigc::bind (sigc::mem_fun (*this, &Editor::align_regions), ARDOUR::SyncPoint));
    reg_sens (_region_actions, "regions-relative-to-session-start", _("Align Relative to Session Start"), sigc::bind (sigc::mem_fun (*this, &Editor::align_regions_relative), ARDOUR::SyncPoint));
...
//	reg_sens (_region_actions, "pitch-shift-region", _("Pitch Shift..."), sigc::mem_fun (*this, &Editor::pitch_shift_region));
	...
	reg_sens (
		_region_actions, "set-loop-from-region", _("Set as Loop Range"), sigc::bind (sigc::mem_fun (*this, &Editor::set_loop_from_region), false)
		);
...
//    reg_sens (_region_actions, "show-rhythm-ferret", _("Rhythm Ferret..."), sigc::mem_fun (*this, &Editor::show_rhythm_ferret));
	...
	reg_sens (_region_actions, "export-region", _("Export Region..."), sigc::mem_fun (*this, &Editor::export_region));
    reg_sens (_region_actions, "import-audio-files", _("Import Audio File..."), sigc::bind (sigc::mem_fun(*this, &Editor::add_external_audio_action), ImportToTrack));
