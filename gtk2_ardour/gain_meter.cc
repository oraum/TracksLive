...
namespace { // helpers
    inline float round_to_tenths (float value) {
    ...
    , _gain_slider_double_clicked (false)
{
    ...
    _cur_peak_display_color = _peak_level_3_color;
    ...
    level_meter.MaxPeakUpdated.connect_same_thread(_max_peak_update_connection, boost::bind (&GainMeter::on_max_peak_update, this, _1) );
    ...
    ResetAllPeakDisplays.connect_same_thread (global_meter_connections, boost::bind(&GainMeter::reset_peak_display, this));
	ResetRoutePeakDisplays.connect_same_thread (global_meter_connections, boost::bind(&GainMeter::reset_route_peak_display, this, _1));
	ResetGroupPeakDisplays.connect_same_thread (global_meter_connections, boost::bind(&GainMeter::reset_group_peak_display, this, _1));
	RedrawMetrics.connect_same_thread (global_meter_connections, boost::bind(&GainMeter::redraw_metrics, this));
...
{
	char buf[32];
	const char *final_buf = buf;

	double v = gain_adjustment.get_value();

	switch (_data_type) {
	case DataType::AUDIO: {
			float gain = accurate_coefficient_to_dB (slider_position_to_gain_with_max (v, Config->get_max_gain()));
			if (gain >= ARDOUR_UI::config()->get_gain_min_threshold()) {
				snprintf (buf, sizeof (buf), "%.1f", gain); 
			} else {
				strcpy (buf, _("-inf"));
			}
		}
	...
	if ((final_buf[0] == '-') && (final_buf[1] == '0') && (final_buf[3] == '0') && (final_buf[4] == 0)) {
		++final_buf;
	}
	gain_display_entry.set_text (final_buf);
	gain_display_button.set_text (final_buf);
}
...
GainMeter::gain_adjusted ()
{
	...
	if (_data_type == DataType::AUDIO) {
        ...
        /* make sure it's rounded to tenth if converted to Db
        so that gain display would display value with corresponds slider position */
        float gain = accurate_coefficient_to_dB (value);
        gain = round_to_tenths (gain);
        value = dB_to_coefficient (gain);
...
GainMeter::get_relative_gain_factor (gain_t val, const RouteList& routes)
{
    ...
    const float min_gain_coefficient_gap = 0.0000003f;
    const float max_gain_coefficient = Config->get_max_gain();
    const float min_gain_coefficient = FLT_MIN;
	    
    if (factor > 0.0f) { // get max factor for selected tracks

	    // we are already on top
	    if (fabs(usable_gain - max_gain_coefficient) < min_gain_coefficient_gap) {
            return 0.0f;
        }
        ...
        for (RouteList::const_iterator i = routes.begin(); i != routes.end(); ++i) {
            ...
            if (fabs(g - max_gain_coefficient) < min_gain_coefficient_gap ) {
                continue;
            }
            
            // if the current factor woulnd't raise this route above maximum
            if ((g + g * factor) < max_gain_coefficient) {
                continue;
            }
            
            // factor is calculated so that it would raise current route to max
            factor = max_gain_coefficient / g - 1.0f;
        }
        ...
        for (RouteList::const_iterator i = routes.begin(); i != routes.end(); ++i) {
            ...
            if (g <= min_gain_coefficient) {
                continue;
            }
            
            if ((g + g * factor) > min_gain_coefficient) {
                continue;
            }
            
            // factor is calculated so that it would lower current route to min
            factor = min_gain_coefficient / g - 1.0f;
        }
    }
...
void
GainMeter::on_max_peak_update (float mpeak)
{
    
    if (mpeak > max_peak) {
        set_numeric_peak_indicator_value (mpeak);
    }
}

void
GainMeter::set_numeric_peak_indicator_value (float value)
{
    std::string string_mpeak;
    Gdk::Color color;
    float mpeak = round_to_tenths(value);
    
    if (mpeak < ARDOUR_UI::config()->get_numeric_peak_min_treshold()) {
        string_mpeak = "-inf";
        color = _peak_level_3_color;
        max_peak = -INFINITY;
    } else {
        max_peak = mpeak;
        mpeak = (mpeak == 0.0f) ? 0.0f : mpeak; // show "0.0" instead of "-0.0"
        
        if (mpeak < ARDOUR_UI::config()->get_meter_peak_2()) {
            color = _peak_level_3_color;
        } else if (mpeak < ARDOUR_UI::config()->get_meter_peak()) {
		...
		if ((buf[0] == '-') && (buf[1] == '0') && (buf[3] == '0') && (buf[4] == 0)) {
	        string_mpeak = buf + 1;
		} else {
			string_mpeak = buf;
		}
    ...
    peak_display_button.set_text (string_mpeak);
    if (_cur_peak_display_color != color) {
        _cur_peak_display_color = color;
        peak_display_button.modify_bg(Gtk::STATE_NORMAL, _cur_peak_display_color);
        peak_display_button.modify_bg(Gtk::STATE_ACTIVE, _cur_peak_display_color);
    }
}

float
GainMeter::get_numeric_peak_indicator_value ()
{
    return max_peak;
}
