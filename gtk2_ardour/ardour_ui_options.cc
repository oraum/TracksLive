...
ARDOUR_UI::parameter_changed (std::string p)
{
	if (p == "external-sync") {
		...
		if (!_session->config.get_external_sync()) {
            ...
            if (editor->get_waves_button ("transport_play_button").get_active ()) {
                // as a result of blinking in slave mode
                // button Play stayed active - set it to inactive state
		...
		bool show = ui_config->get_show_editor_meter();
	} else if (p == "output-auto-connect") {
                update_output_operation_mode_buttons();
	} else if (p == "waveform-scale") {
		ArdourCanvas::WaveView::set_global_logscaled (ui_config->get_waveform_scale() == Logarithmic);
	} else if (p == "waveform-shape") {
		ArdourCanvas::WaveView::set_global_shape (ui_config->get_waveform_shape() == Rectified
...
{ 
    if ( param == "native-file-data-format" ) {
            ...
            tracks_control_panel->refresh_session_frame_rate ();
    } else if (param == "timecode-format") {
            set_fps_timeout_connection ();
