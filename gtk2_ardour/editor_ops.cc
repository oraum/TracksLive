...
Editor::scroll_down_one_track ()
{
	...
	for (TrackViewList::iterator next = track_views.begin(); next != track_views.end(); ++next) {
		if ((*next)->hidden()) {
			continue;
		}
		...
		*/
		...
		if ((*next)->y_position () > top_of_trackviews) {
...
Editor::add_location_mark (framepos_t where)
{
    ...
    if (!loc) {
        ...
        AddMarkerDialog dialog (markername);
        int response = dialog.run();
        
        if (response != WavesDialog::RESPONSE_DEFAULT) {
            return;
        }
        
        // markername could have been changed in AddMarkerDialog
        if (!dialog.get_marker_name().empty())
            markername = dialog.get_marker_name();
...
Editor::regions_to_playhead ()
{
    ...
    if (rs.size() > 1) {
        begin_reversible_command (_("move regions to playhead position"));
    } else {
        begin_reversible_command (_("move region to playhead position"));
    }
    
    framepos_t const position = _session->audible_frame();
    
    for (RegionSelection::const_iterator i = rs.begin(); i != rs.end(); ++i) {
        align_region_internal ((*i)->region(), RegionPoint (SyncPoint), position);
    }
...
void
Editor::add_to_location_list ()
{
    ARDOUR_UI::instance ()->add_selection_to_location_list_dialog (*selection);
}

void
Editor::update_current_selection ()
{
    selection->time.get_location_list_selection ()->update_current_selection_time_and_route_ids ();
}

void
Editor::restore_selection (LocationListSelection* lls)
{    
    TrackViewList tracks_to_select;
    std::list<std::string> routes_id = lls->get_routes_id ();
    ...
    for (TrackViewList::const_iterator i = track_views.begin (); i != track_views.end (); ++i) {
        ...
        if (rtav) {
            for (std::list<std::string>::iterator it = routes_id.begin (); it != routes_id.end (); ++it) {
                if (rtav->route_state_id () == (*it)) {
                    tracks_to_select.push_back (*i);
                    break;
                }
            }
        }
    ...
    selection->time.tracks_in_range = tracks_to_select;
    
    if (selection->time.tracks_in_range.empty ()) {
        return;
    }
    
    selection->set (lls->start (), lls->end ()); ///< This function calls time.clear (). It must be invoked before set_location_list_selection
    selection->time.set_location_list_selection (lls);
}
...
Editor::waveform_zoom_changed()
{
    ...
    if ( fabs(new_adjustment_value - _session->config.get_wave_amplitude_zoom () ) > 0.01 ) {
        _session->config.set_wave_amplitude_zoom (new_adjustment_value);
    }
}
