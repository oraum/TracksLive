...
MarkerDrag::start_grab (GdkEvent* event, Gdk::Cursor* cursor)
{
	...
	{
        _editor->selection->clear_markers (_marker->location ()->is_skip () ? Location::Flags (ARDOUR::Location::IsMark) : Location::Flags (ARDOUR::Location::IsSkip));
    
        Locations::LocationList ll;
        list<Marker*> to_add;
        framepos_t s, e;
        _editor->selection->markers.range (s, e);
        if (_marker->location ()->is_skip ()) {
            e = max (_marker->location ()->end (), e);
        }
        ...
        _editor->session()->locations()->find_all_between (s, e, ll, Location::Flags (_marker->location ()->is_skip () ?
                                                                                      Location::Flags (ARDOUR::Location::IsSkip) :
                                                                                      Location::Flags (ARDOUR::Location::IsMark)));
		...
		for (x = _copied_locations.begin(); x != _copied_locations.end(); ++x) {
			...
			*/
			...
			if (copy->is_mark()) {
				...
				if (type == Move) {
					_editor->snap_to (new_start, RoundDownAlways, true);
					_editor->snap_to (new_end, RoundDownAlways, true);
...
static bool
timeout_set_skipping (Editor* editor, Marker* marker)
{
    editor->toggle_location_skipping_command (marker);
    return false;
}
...
MarkerDrag::finished (GdkEvent* event, bool movement_occurred)
{
	if (!movement_occurred) {
		...
		*/
		...
		case Selection::Set:
            if (_marker->location ()->is_skip ()) {
                timeout_connection = Glib::signal_timeout ().connect (sigc::bind (sigc::ptr_fun (timeout_set_skipping), _editor, _marker),
		...
		case Selection::Toggle: // Cmd + click (ctrl on Windows)
            _editor->selection->clear_markers (_marker->location ()->is_skip () ? Location::Flags (ARDOUR::Location::IsMark) : Location::Flags (ARDOUR::Location::IsSkip));
...
MouseZoomDrag::finished (GdkEvent* event, bool movement_occurred)
{
	if (movement_occurred) {
		...
		if (Keyboard::the_keyboard().key_is_down (GDK_Shift_L)) {
			...
			_editor->temporal_zoom_step (_zoom_out);
		}
	}
...
NoteCreateDrag::finished (GdkEvent*, bool had_movement)
{
	...
	Evoral::Beats const length_beats = max (one_tick, _region_view->region_frames_to_region_beats (length));
