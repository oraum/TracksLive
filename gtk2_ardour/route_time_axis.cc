	...
	, _colorless_bg_color (xml_property (*xml_tree ()->root (), "bgnormal", "#FF0000"))
{
    ...
    ARDOUR_UI::config()->ParameterChanged.connect_same_thread (*this, boost::bind (&RouteTimeAxisView::ui_parameter_changed, this, _1) );
}
...
RouteTimeAxisView::display_route_color ()
{
	...
	selected_track_color_box.modify_bg (STATE_ACTIVE, new_color);
	track_color_box.modify_bg (STATE_NORMAL, new_color);
	track_color_box.modify_bg (STATE_ACTIVE, new_color);
...
bool
RouteTimeAxisView::has_regionviews ()
{
	return _view && _view->contains_views ();
}
...
RouteTimeAxisView::name_entry_changed ()
{
	...
	if (_session->route_name_internal (x)) {
        ...
        std::string ensured_route_name = Route::ensure_track_or_route_name (x, *_session);
		_route->set_name (ensured_route_name);
	} else {
		name_entry.grab_focus ();
	}
}
...
void
RouteTimeAxisView::ui_parameter_changed (string p)
{
