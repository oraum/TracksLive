    ...
    , _amplitude_above_axis(1.0)
{
    ...
    if (session) {
        _amplitude_above_axis = session->config.get_wave_amplitude_zoom () * session->config.get_wave_zoom_factor ();
...
AudioStreamView::setup_rec_box ()
{
	...
	if (_trackview.session()->transport_rolling()) {
		...
		if (!rec_active &&
			...
			rec_rect->set_y1 (child_height () - TimeAxisViewItem::REGION_BOTTOM_OFFSET - TimeAxisViewItem::REGION_TOP_OFFSET + 1.0 );
