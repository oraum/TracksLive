...
Editor::track_selection_changed ()
{
    if (!selection->tracks.empty() ) {
        ...
        bool first_time_axis_view_is_visible = false;
        for (TrackViewList::iterator iter = track_views.begin(); iter != track_views.end(); ++iter) {
            if (selection->selected(*iter) ) {
                TimeAxisView* tv = *iter;
                set_selected_mixer_strip (*tv);
                
                if (first_time_axis_view_is_visible == false) {
                    // make sure first selected track is visible in track header
                    ensure_time_axis_view_is_visible (*tv, false);
                    first_time_axis_view_is_visible = true;
                }
            }
        }
    ...
    if (_session->record_status () == Session::Recording && _session->have_rec_enabled_track ()) {
        ActionManager::set_sensitive (ActionManager::record_and_playback_restricted_actions, false );
    }
...
Editor::time_selection_changed ()
{
    ...
    if (!_session->transport_rolling ()) {
        ...
        ActionManager::set_sensitive (ActionManager::region_or_time_selection_sensitive_actions, allow_invert_regionviews_selection ());
    }
...
{
    if (!_session->transport_rolling ()) {
        // check if we should enable region exists sensitive actions (select all)
        ActionManager::set_sensitive (ActionManager::region_exists_sensitive_actions, has_regionviews ());
        // check if we should enable region or time selection sensitive actions (invert selection)
        ActionManager::set_sensitive (ActionManager::region_or_time_selection_sensitive_actions, allow_invert_regionviews_selection ());
    }
}
...
bool
Editor::allow_invert_regionviews_selection ()
{
    if ( ! has_regionviews ()) {
        return false;
    }
    
    if ( ! selection->regions.empty () ||  ! selection->time.tracks_in_range.empty ()) {
        return true;
    }
...
Editor::invert_selection ()
{
    ...
    /* Do nothing if all regions are selected
       variable touched contains not selected regions */
    if ( ! touched.empty ()) {
        selection->set (touched);
    }
}
