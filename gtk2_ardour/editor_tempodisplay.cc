...
Editor::draw_metric_marks (const Metrics& metrics)
{
	...
	for (Metrics::const_iterator i = metrics.begin(); i != metrics.end(); ++i) {
		...
		if ((ms = dynamic_cast<const MeterSection*>(*i)) != 0) {
			...
			metric_marks.push_back (new MeterMarker (*this, *meter_group, marker_height, ARDOUR_UI::config()->get_canvasvar_MeterMarker(), buf,
			...
			metric_marks.push_back (new TempoMarker (*this, *tempo_group, marker_height, ARDOUR_UI::config()->get_canvasvar_TempoMarker(), buf,
