...
namespace {
    const std::string tooltip_record_disabled = "Record Disabled";
    const std::string tooltip_record_enabled  = "Record Enabled";
    const std::string tooltip_record_safe    = "Record Safe";
}
...
RouteUI::set_route (boost::shared_ptr<Route> rp)
{
	...
	if (_session && _session->writable() && is_track()) {
		...
		t->RecordSafeChanged.connect (route_connections, invalidator (*this), boost::bind (&RouteUI::route_rec_safe_changed, this), gui_context());
...
/*
 Return all routes except master
 */
boost::shared_ptr<ARDOUR::RouteList>
RouteUI::get_not_master_routes ()
{
    boost::shared_ptr<ARDOUR::RouteList> route_list (new RouteList);
    for (RouteList::iterator it = _session->get_routes ()->begin (); it != _session->get_routes ()->end (); ++it) {
        if (!(*it)->is_master ()) {
            route_list->push_back (*it);
        }
    }
...
//    Groups will be able to use in the future, but Tracks Live doesn't support it now
    ...
    if (Keyboard::modifier_state_equals (ev->state, alt_modifier)) {
        ...
        bool current_track_is_muted = _route->muted ();
        boost::shared_ptr<RouteList> rl (new RouteList);
        rl = get_not_master_routes ();
        
        // get tracks to invert mute property
        boost::shared_ptr<RouteList> rl_to_invert (new RouteList);
        for (RouteList::iterator i = rl->begin(); i != rl->end(); ++i) {
            if ((*i)->muted () == current_track_is_muted) {
                rl_to_invert->push_back (*i);
            }
        }

        _session->set_mute (rl_to_invert, !_route->muted ());
    } else {
        if (is_selected ()) {
    ...
    if (Keyboard::is_context_menu_event (ev)) {
		...
		if ( ev->button == 1 )
        {
            if (Keyboard::modifier_state_equals (ev->state, control_modifier)) {
                ...
                if ( is_selected () ) {
                    boost::shared_ptr<ARDOUR::RouteList> selected_route_list = get_selected_route_list ();
                    // XXX API not yet present in libardour 
                    _session->set_solo_isolated (selected_route_list, !_route->solo_isolated(), Session::rt_cleanup, true);
                } else {
	                // XXX API not yet present in libardour 
	                _route->set_solo_isolated (!_route->solo_isolated(), _session);
                }
			} else
            if (Keyboard::modifier_state_equals (ev->state, alt_modifier)) {
                ...
                bool track_is_soloed = Config->get_solo_control_is_listen_control () ? _route->listening_via_monitor() : _route->self_soloed ();
                // if track is soloed just clear solo (previous step)
...
#endif
            if (Keyboard::modifier_state_equals (ev->state, alt_modifier)) {
                // prohibit to switch directly from Record Save to Record Enable mode
                if (_route->record_safe ()) {
                    return false;
                }
                
                bool current_track_is_record_enabled = rec_enable_button.active_state ();
                boost::shared_ptr<RouteList> rl (new RouteList);
                rl = get_not_master_routes ();
                // get tracks to invert
                boost::shared_ptr<RouteList> rl_to_invert (new RouteList);
                for (RouteList::iterator i = rl->begin(); i != rl->end(); ++i) {
                    if ((*i)->record_enabled () == current_track_is_record_enabled) {
                        rl_to_invert->push_back (*i);
                    }
                }
                ...
                DisplaySuspender ds;
                _session->set_record_enabled (rl_to_invert, !current_track_is_record_enabled, Session::rt_cleanup, true);
            } else
            if (Keyboard::modifier_state_contains (ev->state, control_modifier)) {
                // control-click: toggle record safe status
                boost::shared_ptr<RouteList> rl (new RouteList);
                if (is_selected ()) {
                    rl = get_selected_route_list ();
                } else {
                ...
                // if (session is recording) OR (transport is rolling) OR (current clicked route is in record safe state)
                // then do not manipulate with Record Enabled tracks
                if ((_session->record_status () == Session::Recording) || (_session->transport_rolling()) || (_route->record_safe ())) {
                    // get not enabled tracks
                    boost::shared_ptr<RouteList> rl_not_enabled (new RouteList);
                    for (RouteList::iterator i = rl->begin(); i != rl->end(); ++i) {
                        if (!(*i)->record_enabled ())
                            rl_not_enabled->push_back (*i);
                    }
                    rl = rl_not_enabled;
                }
                
                DisplaySuspender ds;
                _session->set_record_safe (rl, !_route->record_safe (), Session::rt_cleanup, true);
            } else {
                // prohibit to switch directly from Record Save to Record Enable mode
                if (_route->record_safe ()) {
                    return false;
...
RouteUI::step_edit_changed (bool yn)
{
    if (yn) {
        ...
        rec_enable_button.set_tooltip_text (tooltip_record_enabled);
        start_step_editing ();
        if (step_edit_item) {
        ...
        rec_enable_button.set_tooltip_text (tooltip_record_disabled);
        stop_step_editing ();
        if (step_edit_item) {
...
void
RouteUI::route_rec_safe_changed ()
{
    if (_route->record_safe ()) {
        rec_enable_button.set_active_state (Gtkmm2ext::ImplicitActive);
        rec_enable_button.set_tooltip_text (tooltip_record_safe);
    } else {
...
RouteUI::set_color (const Gdk::Color & c)
{
	...
	*/
	...
	gui_object_state().set_property<std::string> (route_state_id(), X_("color"), buf);
...
RouteUI::remove_this_route (bool apply_to_selection)
{
    ...
    if (apply_to_selection) {      
        ...
        for (std::list<TimeAxisView*>::iterator i = track_selection.begin(); i != track_selection.end(); ++i) {
