	...
	, _no_tracks_changed (false)
{
    ...
    void (Selection::*location_list_item_remove)(LocationListSelection*) = &Selection::remove;
    LocationListSelection::CatchDeletion.connect (*this, MISSING_INVALIDATOR, boost::bind (location_list_item_remove, this, _1), gui_context ());
    
    time.set_location_list_selection (0);
    time.set_ignore_lls_change (false);
}
...
void
Selection::clear_markers (ARDOUR::Location::Flags flag)
{
    MarkerSelection::iterator i = markers.begin ();
    while (i != markers.end ()) {
        if ((*i)->location()->flags () & flag) {
            i = markers.erase (i);
            continue;
        }
        ++i;
...
Selection::remove (TimeAxisView* track)
{
    ...
    if ((i = find (time.tracks_in_range.begin (), time.tracks_in_range.end (), track)) != time.tracks_in_range.end ()) {
        time.tracks_in_range.erase (i);
    }
}
...
void
Selection::remove (LocationListSelection* lls)
{
    if (time.get_location_list_selection () == lls) {
        time.set_location_list_selection (0);
    }
}
...
{
    clear_markers (m->location ()->is_skip () ? ARDOUR::Location::IsMark : ARDOUR::Location::IsSkip);
