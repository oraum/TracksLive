...
MidiRegionView::enter_notify (GdkEventCrossing* ev)
{
	...
	if (trackview.editor().current_mouse_mode() == MouseGain) {
		...
		editor->push_canvas_cursor(editor->cursors()->timebar);
	}
...
MidiRegionView::button_press (GdkEventButton* ev)
{
	...
	if (m == MouseObject && Keyboard::modifier_state_contains (ev->state, Keyboard::insert_note_modifier())) {
		editor->push_canvas_cursor (editor->cursors()->midi_pencil);
	} 
...
MidiRegionView::button_release (GdkEventButton* ev)
{
	...
	dynamic_cast<Editor*>(&editor)->pop_canvas_cursor ();
		...
		case MouseTimeFX:
			{
				...
				if (Keyboard::is_insert_note_event(ev)) {
					...
					bool success;
					Evoral::Beats beats = editor.get_grid_type_as_beats (success, editor.pixel_to_sample (event_x));
...
MidiRegionView::nudge_notes (bool forward)
{
	...
	*/
	...
	Evoral::Beats delta = region_frames_to_region_beats (fabs ((double)distance));
...
MidiRegionView::paste (framepos_t pos, float times, const MidiCutBuffer& mcb)
{
	...
	Evoral::Beats beat_delta;
	Evoral::Beats paste_pos_beats;
	Evoral::Beats duration;
	Evoral::Beats end_point;
...
MidiRegionView::update_ghost_note (double x, double y)
{
	...
	*/
	...
	bool success;
	Evoral::Beats length = editor.get_grid_type_as_beats (success, unsnapped_frame);

	if (!success) {
		length = Evoral::Beats (1);
	}
...
MidiRegionView::snap_frame_to_grid_underneath (framepos_t p, framecnt_t& grid_frames) const
{
	...
	bool success;
	Evoral::Beats grid_beats = editor.get_grid_type_as_beats (success, p);
