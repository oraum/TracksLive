	...
	, drag_field (Field (0))
{
    ...
    EngineStateController::instance()->SampleRateChanged.connect (update_connections, MISSING_INVALIDATOR, boost::bind (&AudioClock::update_time, this), gui_context());
}

void
AudioClock::update_time ()
{
    if (_session) {
        // force a redraw. last_when will be preserved, but the clock text will change
        set (last_when, true);
    }
}
...
AudioClock::render (cairo_t* cr, cairo_rectangle_t* expose_area)
{
	...
	if (_need_bg) {
		...
		if (corner_radius) {
			if (_left_layout) {
				Gtkmm2ext::rounded_top_half_rectangle (cr, origx, origy, get_width(), upper_height, corner_radius);
			} else {
				Gtkmm2ext::rounded_rectangle (cr, origx, origy, get_width(), upper_height, corner_radius);
			}
		} else {
			cairo_rectangle (cr, origx, origy, get_width(), upper_height);
		}
		cairo_fill (cr);
	}

	cairo_move_to (cr, 
				   (get_width() - layout_width) / 2.0 + origx,
				   (upper_height - layout_height) / 2.0 + origy);
	...
	if (_left_layout) {
		...
		if (mode_based_info_ratio != 1.0) {
			...
			if (_need_bg) {
				if (corner_radius) {
					Gtkmm2ext::rounded_bottom_half_rectangle (cr,
															  origx, 
															  origy + upper_height + separator_height,
			...
			cairo_move_to (cr,
						   origx + x_leading_padding,
						   origy + upper_height + separator_height + ((h - info_height)/2.0));
			pango_cairo_show_layout (cr, _left_layout->gobj());

			if (_need_bg) {
				if (corner_radius) {
					Gtkmm2ext::rounded_bottom_half_rectangle (cr,
															  origx + left_rect_width + separator_height,
															  origy + upper_height + separator_height,
			...
			if (_right_layout->get_alignment() == Pango::ALIGN_RIGHT) {
				...
				cairo_move_to (cr,
							   origx + x_leading_padding + left_rect_width + separator_height,
							   origy + upper_height + separator_height + ((h - info_height)/2.0));
			}
	...
	if (editing) {
		if (!insert_map.empty()) {
			...
			if (input_string.length() < insert_map.size()) {
				...
				cairo_rectangle (cr,
								 origx + min (get_width() - 2.0, (double) xcenter + cursor.get_x()/PANGO_SCALE + em_width),
								 origy + (upper_height - layout_height)/2.0,
								 2.0,
...
AudioClock::end_edit (bool modify)
{
	if (modify) {
		...
		case Frames:
            // Restore previous value if edit_string is empty
            // But leave ok == true to end editing
            if (edit_string.empty() ) {
                edit_string = pre_edit_string;
            } else {
                ok = frames_validate_edit (edit_string);
            }
			break;
		}
  ...
  use_input_string:
	...
	case Frames:
		...
		if (ev->keyval == GDK_Delete || ev->keyval == GDK_BackSpace) {
            ...
            if ( ! frames_validate_edit (edit_string)) {
                edit_string = edit_string.substr (0, edit_string.length() - 1);
            }
		}
		if (!edit_string.empty()) {
			char buf[32];
			int result = sscanf (edit_string.c_str(), "%" PRId64, &pos);
            if (result <= 0) {
                edit_string.clear();
            } else {
...
bool
AudioClock::frames_validate_edit (const std::string& edit_string)
{
    static const framepos_t seconds_in_a_day = 24 * 60 * 60;
    framepos_t max_pos = seconds_in_a_day * _session->frame_rate () - 1;
	...
	framepos_t f;
	sscanf (edit_string.c_str(), "%" PRId64, &f);
