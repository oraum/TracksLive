	...
	, _read_only (read_only)
{
	...
	Gtk::EventBox::set_size_request(_face_pixbuf->get_width(), _face_pixbuf->get_height());
	set_can_focus (true);
}
...
bool
Fader::on_expose_event (GdkEventExpose *evt)
{
	int origx(0), origy(0);
	...
	if (_underlay_pixbuf != 0) {
		int upb_width = _underlay_pixbuf->get_width() * xscale;
		int upb_height = _underlay_pixbuf->get_height() * yscale;
		
		if (abs(_max_pos_x - _min_pos_x) > abs(_max_pos_y - _min_pos_y)) {
			upb_width /= 2;
		} else {
			upb_height /= 2;
		}
		
		int x = origx + (get_width () - upb_width) / 2;
		int y = origy + (get_height () - upb_height) / 2;

		gdk_cairo_set_source_pixbuf (cr->cobj (),
		                             _underlay_pixbuf->gobj(),
		                             (origx + _last_drawn_x - (int)((_underlay_pixbuf->get_width() * xscale) / 2.0 + 0.5)) / xscale,
		                             (origy + _last_drawn_y - (int)((_underlay_pixbuf->get_height() * yscale) / 2.0 + 0.5)) / yscale);
		cr->rectangle (x/xscale, y/yscale, upb_width/xscale, (upb_height)/yscale);
		cr->fill ();
	}
	...
	gdk_cairo_set_source_pixbuf (cr->cobj (),
	                             handle_pixbuf->gobj(),
	                             (origx + _last_drawn_x - (int)((handle_pixbuf->get_width() * xscale) / 2.0 + 0.5)) / xscale,
	                             (origy + _last_drawn_y - (int)((handle_pixbuf->get_height() * yscale) / 2.0 + 0.5)) / yscale);
	cr->paint ();
	return true;
}
