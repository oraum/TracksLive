...
WavesAudioBackend::set_port_name (PortHandle port_handle, const std::string& port_name)
{
    ...
    if (!_registered (port_handle)) {
        PBD::error << "WavesAudioBackend::set_port_name (): Failed to find port [" << port_handle << "]!" << endmsg;
        return -1;
    }
...
WavesAudioBackend::get_port_name (PortHandle port_handle) const
{
    ...
    if (!_registered (port_handle)) {
        PBD::error << "WavesAudioBackend::get_port_name (): Failed to find port [" << port_handle << "]!" << endmsg;
        return std::string ();
    }
...
WavesAudioBackend::get_port_by_name (const std::string& port_name) const
{
    ...
    if (!port_handle) {
        PBD::error << "WavesAudioBackend::get_port_by_name (): Failed to find port [" << port_name << "]!" << endmsg;
    }
...
WavesAudioBackend::port_data_type (PortHandle port_handle) const
{
    ...
    if (!_registered (port_handle)) {
        PBD::error << "WavesAudioBackend::port_data_type (): Failed to find port [" << port_handle << "]!" << endmsg;
        return DataType::NIL;
    }
...
WavesAudioBackend::register_port (const std::string& shortname, ARDOUR::DataType type, ARDOUR::PortFlags flags)
{
    ...
    if (shortname.size () == 0) {
        PBD::error << "WavesAudioBackend::register_port (): Invalid (empty) port name!" << endmsg;
        return NULL;
    }

    if (flags & IsPhysical) {
        PBD::error << "WavesAudioBackend::register_port (): Unexpected attribute for port [" << shortname << "]! The port must not be physical!";
        return NULL;
    }
...
WavesAudioBackend::_register_port (const std::string& port_name, ARDOUR::DataType type, ARDOUR::PortFlags flags)
{
    ...
    if (_find_port (port_name) != NULL) {
        PBD::error << "WavesAudioBackend::register_port () : Port [" << port_name << "] is already registered!" << endmsg;
        return NULL;
    }
    ...
    switch (type) {
        ...
        default:
            PBD::error << "WavesAudioBackend::register_port () : Invalid data type (" << (uint32_t)type << ") applied to port [" << port_name << "]!" << endmsg;
        return NULL;
    }
...
WavesAudioBackend::unregister_port (PortHandle port_handle)
{
    ...
    if (port_iterator == _ports.end ()) {
        PBD::error << "WavesAudioBackend::unregister_port (): Failed to find port [" << port_handle << "]!"  << endmsg;
        return;
    }
        ...
        if (port->is_output ()) {
            switch (port->type ()) {
                case ARDOUR::DataType::AUDIO: {
                    ...
                    if (audio_port_iterator == _physical_audio_inputs.end ())    {
                        PBD::error << "WavesAudioBackend::unregister_port (): Failed to find port [" << port->name () << "] in the list of registered physical audio inputs!" << endmsg;
                        return;
                    }
                ...
                case ARDOUR::DataType::MIDI: {
                    ...
                    if (midi_port_iterator == _physical_midi_inputs.end ()) {
                        PBD::error << "WavesAudioBackend::unregister_port (): Failed to find port [" << port->name () << "] in the list of registered physical midi inputs!" << endmsg;
                        return;
                    }
                ...
                break;
                default:
                    PBD::error << "WavesAudioBackend::unregister_port (): Invalid type (" << port->type () << " applied to [" << port->name () << "]!" << endmsg;
                break;
            }
        }
            ...
            switch (port->type ()) {
                case ARDOUR::DataType::AUDIO: {
                    ...
                    {
                        PBD::error << "WavesAudioBackend::unregister_port: Failed to find port [" << port->name () << "] in the list of registered physical audio outputs!\n";
                        return;
                    }
                ...
                case ARDOUR::DataType::MIDI: {
                    ...
                    {
                        PBD::error << "WavesAudioBackend::unregister_port: Failed to find port [" << port->name () << "] in the list of registered physical midi outputs!\n";
                        return;
                    }
...
WavesAudioBackend::connect (const std::string& src_port_name, const std::string& dst_port_name)
{
    ...
    if (src_port == NULL) {
        PBD::error << "WavesAudioBackend::connect: Failed to find source port " << src_port_name << " !" << endmsg;
        return -1;
    }
    
    WavesDataPort* dst_port = _find_port (dst_port_name);
    if (dst_port == NULL) {
        PBD::error << "WavesAudioBackend::connect: Failed to find destination port " << dst_port_name << " !" << endmsg;
        return -1;
    }
...
WavesAudioBackend::connect (PortHandle src_port_handle, const std::string& dst_port_name)
{
    ...
    if (!_registered (src_port_handle)) {
        PBD::error << "WavesAudioBackend::connect: Failed to find source port [" << src_port_handle << "]!" << endmsg;
        return -1;
    }
...
WavesAudioBackend::disconnect (PortHandle src_port_handle, const std::string& dst_port_name)
{
    ...
    if (!_registered (src_port_handle)) {
        PBD::error << "WavesAudioBackend::disconnect (): Failed to find source port [" << src_port_handle << "]!" << endmsg;
        return -1;
    }
    ...
    WavesDataPort* dst_port = _find_port (dst_port_name);
    if (dst_port == NULL) {
        PBD::error << "WavesAudioBackend::disconnect (): Failed to find destination port [" << dst_port_name << "]!" << endmsg;
        return -1;
    }
...
WavesAudioBackend::disconnect_all (PortHandle port_handle)
{
    ...
    if (!_registered (port_handle)) {
        PBD::error << "WavesAudioBackend::disconnect_all : Failed to find port [" << port_handle << "]!" << endmsg;
        return -1;
    }
...
WavesAudioBackend::disconnect (const std::string& src_port_name, const std::string& dst_port_name)
{
    ...
    WavesDataPort* src_port = _find_port (src_port_name);
    if (src_port == NULL) {
        PBD::error << "WavesAudioBackend::disconnect : Failed to find source port!\n";
        return -1;
    }
    
    WavesDataPort* dst_port = _find_port (dst_port_name);
    if (dst_port == NULL) {
        PBD::error << "WavesAudioBackend::disconnect : Failed to find destination port!\n";
        return -1;
    }
...
WavesAudioBackend::connected (PortHandle port_handle, bool process_callback_safe)
{
    ...
    if (!_registered (port_handle)) {
        PBD::error << "WavesAudioBackend::connected (): Failed to find port [" << port_handle << "]!" << endmsg;
        return false;
    }
...
WavesAudioBackend::connected_to (PortHandle src_port_handle, const std::string& dst_port_name, bool process_callback_safe)
{
    ...
    if (!_registered (src_port_handle)) {
        PBD::error << "WavesAudioBackend::connected_to : Failed to find source port!" << endmsg;
        return false;
    }

    WavesDataPort* dst_port = _find_port (dst_port_name);
    if (dst_port == NULL) {
        PBD::error << "WavesAudioBackend::connected_to : Failed to find destination port!" << endmsg;
        return -1;
    }
...
WavesAudioBackend::physically_connected (PortHandle port_handle, bool process_callback_safe)
{
    ...
    if (!_registered (port_handle)) {
        PBD::error << "WavesAudioBackend::physically_connected (): Failed to find port [" << port_handle << "]!" << endmsg;
        return false;
    }
...
WavesAudioBackend::get_connections (PortHandle port_handle, std::vector<std::string>& names, bool process_callback_safe)
{
   // COMMENTED DBG LOGS */ std::cout  << "WavesAudioBackend::get_connections ()" << std::endl;
    ...
    if (!_registered (port_handle)) {
        PBD::error << "WavesAudioBackend::get_connections (): Failed to find port [" << port_handle << "]!" << endmsg;
        return -1;
    }

    if (names.size ()) {
        PBD::error << "WavesAudioBackend::get_connections () : Parameter 'names' is not empty!\n";
        return -1;
    }
...
WavesAudioBackend::port_is_physical (PortHandle port_handle) const
{
    ...
    if (!_registered (port_handle)) {
        PBD::error << "WavesAudioBackend::port_is_physical (): Failed to find port [" << port_handle << "]!" << endmsg;
        return -1;
    }
...
WavesAudioBackend::get_buffer (PortHandle port_handle, pframes_t nframes)
{
    ...
    if (port_handle == NULL) {
        PBD::error << "WavesAudioBackend::get_buffer : Invalid port handler <NULL>!" << endmsg;
        return NULL;
    }  
...
{
    if (!_device) {
        PBD::error << "WavesAudioBackend::_register_system_audio_ports (): No device is set!" << endmsg;
        return -1;
    }
    ...
    for (std::vector<std::string>::iterator it = input_channels.begin (); 
        ...
        if (port == NULL) {
            PBD::error << "WavesAudioBackend::_create_system_audio_ports (): Failed registering port [" << port_name << "] for [" << _device->DeviceName () << "]" << endmsg;
            return-1;
        }
...
WavesAudioBackend::_unregister_system_audio_ports ()
{
    ...
    for (std::vector<WavesAudioPort*>::const_iterator it = physical_audio_ports.begin (); it != physical_audio_ports.end (); ++it) {
        ...
        if (port_iterator == _ports.end ()) {
            PBD::error << "WavesAudioBackend::_unregister_system_audio_ports (): Failed to find port [" << (*it)->name () << "]!"  << endmsg;
        }
        else {
            _ports.erase (port_iterator);
        }
