...
WTErr WCMRCoreAudioDevice::UpdateDeviceInputs()
{
    ...
    for (int channel = 1; channel <= maxInputChannels; channel++)
    {
...
WTErr WCMRCoreAudioDevice::UpdateDeviceOutputs()
{
    ...
    for (int channel = 1; channel <= maxOutputChannels; channel++)
    {
        CFStringRef cfName (0);
        std::stringstream chNameStream;
        UInt32 nameSize = 0;
...
WTErr WCMRCoreAudioDevice::SetAndCheckCurrentSamplingRate (int newRate)
{
    ...
    else
    {
        ...
        if (tryAgain != 0)
        {
            ...
            char debugMsg[128];
            snprintf (debugMsg, sizeof(debugMsg), "Actual Wait for SR Change (%d) was %d milliseconds", newRate, actualWait * PROPERTY_CHANGE_SLEEP_TIME_MILLISECONDS);
...
void WCMRCoreAudioDevice::PropertyChangeProc (AudioDevicePropertyID inPropertyID)
{
    ...
    case kAudioDevicePropertyNominalSampleRate:
        {
            m_SRChangeRequested++;
            CurrentSamplingRate (); // update current sampling rate
        }
        break;
#if ENABLE_DEVICE_CHANGE_LISTNER    
...
WCMRAudioDevice* WCMRCoreAudioDeviceManager::initNewCurrentDeviceImpl(const std::string & deviceName)
{
    ...
    PBD::info << "API::CoreAudioDeviceManager::initNewCurrentDevice " << deviceName << endmsg;
	...
	if (eNoErr == err)
	{
		try
		{
			PBD::info << "API::CoreAudioDeviceManager::Creating PA device: " << devInfo.m_DeviceId << ", Device Name: " << devInfo.m_DeviceName << endmsg;
			TRACE_MSG ("API::CoreAudioDeviceManager::Creating PA device: " << devInfo.m_DeviceId << ", Device Name: " << devInfo.m_DeviceName);
...
WTErr WCMRCoreAudioDeviceManager::updateDeviceListImpl()
{
    ...
    if (eNoErr != err)
    {
        PBD::info << "API::CoreAudioDeviceManager::updateDeviceListImpl: Device list update error: "<< err << endmsg;
    }
    ...
    if (m_CurrentDevice)
    {
        ...
        {
            // reset device unconditionaly in this case
            bool force_reset = true;
            NotifyClient (WCMRAudioDeviceManagerClient::RequestReset, (void*)&force_reset);
            return err;
        }
    }
...
WTErr WCMRCoreAudioDeviceManager::getDeviceBufferSizesImpl(const std::string & deviceName, std::vector<int>& bufferSizes) const
{
    ...
    else
	{
		...
		PBD::info << "API::CoreAudioDeviceManager::GetBufferSizes: Device not found: "<< deviceName << endmsg;
	}
