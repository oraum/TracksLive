...
WCMRAudioDeviceManager::~WCMRAudioDeviceManager()
{
	...
	PBD::info << "API::Destroying AudioDeviceManager " << endmsg;
	try
	{
...
{
	WCMRAudioDevice* device = initNewCurrentDeviceImpl(deviceName);
#ifdef PLATFORM_WINDOWS
	// try again, device may be removed
	if (device == NULL) {
		generateDeviceListImpl();
		device = initNewCurrentDeviceImpl(deviceName);
	}
#endif
	return device;
}
