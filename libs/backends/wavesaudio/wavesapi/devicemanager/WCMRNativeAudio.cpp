...
WTErr WCMRNativeAudioNoneDevice::SetStreaming (bool newState)
{
	...
	if (Streaming())
	{
		if (is_pthread_active (m_SilenceThread))
			PBD::warning << "WCMRNativeAudioNoneDevice::SetStreaming (): The inactive device was streaming!" << endmsg;
		...
		if (pthread_attr_init (&attributes)) {
			PBD::error << "WCMRNativeAudioNoneDevice::SetStreaming (): pthread_attr_init () failed!" << endmsg;
			return eGenericErr;
		}
   
		if (pthread_attr_setstacksize (&attributes, stack_size)) {
			PBD::error << "WCMRNativeAudioNoneDevice::SetStreaming (): pthread_attr_setstacksize () failed!" << endmsg;
			return eGenericErr;
		}

		if (pthread_create (&m_SilenceThread, &attributes, __SilenceThread, this)) {
			mark_pthread_inactive (m_SilenceThread);
			PBD::error << "WCMRNativeAudioNoneDevice::SetStreaming (): pthread_create () failed!" << endmsg;
			return eGenericErr;
		}
	}
	else
	{
        bool is_active = is_pthread_active (m_SilenceThread);
		if (!is_active)
		{
			PBD::warning << "WCMRNativeAudioNoneDevice::SetStreaming (): The active device was NOT streaming!" << endmsg;
		}
