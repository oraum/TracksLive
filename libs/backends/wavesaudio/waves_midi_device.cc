		...
		if (!_input_pm_stream) {
			...
			if (!_input_queue) {
				...
				if (NULL == _input_queue) {
                    PBD::error << "WavesMidiDevice::open (): _input_queue = Pm_QueueCreate () failed for " << _pm_input_id << "-[" << name () <<  "]!" << endmsg;
                    return -1;
				}
			} 
			// create stream
            ...
            if (pmNoError != Pm_OpenInput (&_input_pm_stream, 
					...
					PBD::error << "WavesMidiDevice::open (): Pm_OpenInput () failed for " << _pm_input_id << "-[" << name () <<  "]!\n    Port Midi Host Error: " << err_msg << endmsg;
					close ();
                    return -1;
            }
			// COMMENTED DBG LOGS */ std::cout << "    DONE : " << std::endl;
		...
		if (!_output_pm_stream) {
			...
			if (!_output_queue) {
				...
				if (NULL == _output_queue) {
					PBD::error << "WavesMidiDevice::open (): _output_queue = Pm_QueueCreate () failed for " << _pm_output_id << "-[" << name () <<  "]!" << endmsg;
					return -1;
				}
			}
			// create stream
			...
			if (pmNoError != Pm_OpenOutput (&_output_pm_stream, 
				...
				char* err_msg = new char[256];
				Pm_GetHostErrorText(err_msg, 256);
				PBD::error << "WavesMidiDevice::open (): Pm_OpenOutput () failed for " << _pm_output_id << "-[" << name () <<  "]!\n    Port Midi Host Error: " << err_msg << endmsg;
				close ();
				return -1;
			}
			// COMMENTED DBG LOGS */ std::cout << "    DONE : " << std::endl;
...
WavesMidiDevice::close ()
{
	...
	if (input_pm_stream) {
		...
		if (err != pmNoError) {
			char* err_msg = new char[256];
			Pm_GetHostErrorText(err_msg, 256);
			PBD::error << "WavesMidiDevice::close (): Pm_Close (input_pm_stream) failed (" << err << ") for " << input_pm_stream << "-[" << name () <<  "]!\n    Port Midi Host Error: " << err_msg << endmsg;
		}
		_pm_input_id = pmNoDevice;
	}
	...
	if ( output_pm_stream ) {
		...
		if (err != pmNoError) {
			char* err_msg = new char[256];
			Pm_GetHostErrorText(err_msg, 256);
			PBD::error << "WavesMidiDevice::close (): Pm_Close (output_pm_stream) failed (" << err << ") for " << output_pm_stream << "-[" << name () <<  "]!\n    Port Midi Host Error: " << err_msg << endmsg;
		}
		_pm_output_id = pmNoDevice;
	}
        ...
        while (Pm_Poll (_input_pm_stream) > 0) {
                ...
                case WavesMidiEvent::COMPLETE:
						...
						if (pmNoError != Pm_Enqueue (_input_queue, &_incomplete_waves_midi_event) ) {
							char* err_msg = new char[256];
							Pm_GetHostErrorText(err_msg, 256);
							PBD::error << "WavesMidiDevice::read_midi (): Pm_Enqueue () failed for [" << name () <<  "]!\n    Port Midi Host Error: " << err_msg << endmsg;
						}
...
WavesMidiDevice::write_midi ()
{
        ...
        while (1 == Pm_Dequeue (_output_queue, &waves_midi_event)) {
                if (waves_midi_event->sysex ()) {
                        ...
                        if (0 > err) {
                                PBD::error << "WavesMidiDevice::write_event_to_device (): [" << name () << "] Pm_WriteSysEx () failed (" << err << ")!" << endmsg;
                        };
