...
WavesAudioBackend::set_latency_range (PortHandle port_handle, bool for_playback, LatencyRange latency_range)
{
    if (!_registered (port_handle)) {
        PBD::error << "WavesAudioBackend::set_latency_range (): Failed to find port [" << std::hex << port_handle << std::dec << "]!" << endmsg;
        return;
    }
...
{
    if (!_registered (port_handle)) {
        PBD::error << "WavesAudioBackend::get_latency_range (): Failed to find port [" << std::hex << port_handle << std::dec << "]!" << endmsg;
        LatencyRange lr = {0,0};
        return lr;
    }   
