...
void WavesAudioBackend::AudioDeviceManagerNotification (NotificationReason reason, void* parameter)
{
    switch (reason) {
        case WCMRAudioDeviceManagerClient::DeviceDebugInfo:
            PBD::info << "WCMRAudioDeviceManagerClient::DeviceDebugInfo -- " << (char*)parameter << endmsg;
            break;
        case WCMRAudioDeviceManagerClient::BufferSizeChanged:
            PBD::info << "WCMRAudioDeviceManagerClient::BufferSizeChanged: " << *(int*)parameter << endmsg;
			_buffer_size_change(*(int*)parameter);
            break;
        case WCMRAudioDeviceManagerClient::RequestReset:
            PBD::info << "WCMRAudioDeviceManagerClient::RequestReset" << endmsg;
            engine.request_backend_reset(*(bool*)parameter);
            break;
        case WCMRAudioDeviceManagerClient::RequestResync:
            PBD::info << "WCMRAudioDeviceManagerClient::RequestResync" << endmsg;
            break;
        case WCMRAudioDeviceManagerClient::SamplingRateChanged:
            PBD::info << "WCMRAudioDeviceManagerClient::SamplingRateChanged: " << *(float*)parameter << endmsg;
			_sample_rate_change(*(float*)parameter);
            break;
        case WCMRAudioDeviceManagerClient::Dropout:
            PBD::info << "WCMRAudioDeviceManagerClient::Dropout: " << endmsg;
            break;
        case WCMRAudioDeviceManagerClient::DeviceDroppedSamples:
            PBD::info << "WCMRAudioDeviceManagerClient::DeviceDroppedSamples" << endmsg;
            break;
        case WCMRAudioDeviceManagerClient::DeviceStoppedStreaming:
            PBD::info << "WCMRAudioDeviceManagerClient::DeviceStoppedStreaming" << endmsg;
            break;
		case WCMRAudioDeviceManagerClient::DeviceStartsStreaming:
            PBD::info << "WCMRAudioDeviceManagerClient::DeviceStartsStreaming" << endmsg;
        ...
        case WCMRAudioDeviceManagerClient::DeviceConnectionLost:
            PBD::info << "WCMRAudioDeviceManagerClient::DeviceConnectionLost" << endmsg;
			engine.request_device_connection_loss_handling ();
            break;
        case WCMRAudioDeviceManagerClient::DeviceListChanged:
            PBD::info << "WCMRAudioDeviceManagerClient::DeviceListChanged" << endmsg;
            engine.request_device_list_update();
            break;
        case WCMRAudioDeviceManagerClient::IODeviceDisconnected:
            PBD::info << "WCMRAudioDeviceManagerClient::IODeviceDisconnected" << endmsg;
            engine.request_device_list_update();
            break;
		case WCMRAudioDeviceManagerClient::DeviceGenericError:
            PBD::info << "WCMRAudioDeviceManagerClient::DeviceGenericError" << endmsg;
...
WavesAudioBackend::available_sample_rates (const std::string& device_name) const
{
	...
	if (eNoErr != retVal) {
        PBD::error << "WavesAudioBackend::available_sample_rates (): Failed to find device [" << device_name << "]" << endmsg;
...
WavesAudioBackend::available_buffer_sizes (const std::string& device_name) const
{
    ...
    if (retVal != eNoErr) {
        PBD::error << "WavesAudioBackend::available_buffer_sizes (): Failed to get buffer size for device [" << device_name << "]" << endmsg;
...
WavesAudioBackend::available_input_channel_count (const std::string& device_name) const
{
	...
	if (eNoErr != err) {
        PBD::error << "WavesAudioBackend::available_input_channel_count (): Failed to find device [" << device_name << "]" << endmsg;
        return 0;
    }
...
WavesAudioBackend::available_output_channel_count (const std::string& device_name) const
{
	...
	if (eNoErr != err) {
        PBD::error << "WavesAudioBackend::available_output_channel_count (): Failed to find device [" << device_name << "]" << endmsg;
        return 0;
    }
...
WavesAudioBackend::set_device_name (const std::string& device_name)
{
    ...
    if (_ports.size ()) {
        PBD::error << "WavesAudioBackend::set_device_name (): There are unregistered ports left after [" << (_device ? _device->DeviceName () : std::string ("<NULL>")) << "]!" << std::endl;
        for (size_t i = 0; i < _ports.size (); ++i) {
            PBD::error << "\t[" << _ports[i]->name () << "]!" << std::endl;
        }
		PBD::error << endmsg;
        return -1;
    }
	...
	if (_device && _device->Streaming () ) {
		PBD::error << "WavesAudioBackend::set_device_name (): [" << _device->DeviceName () << "] is streaming! Current device must be stopped before setting another device as current" << endmsg;
	}
    ...
    if (_device) {
        ...
        if (retVal != eNoErr) {
            PBD::error << "WavesAudioBackend::set_device_name (): [" << _device->DeviceName () << "]->SetActive (false) failed!" << endmsg;
            return -1;
        }
    }
    ...
    if (!device) {
        PBD::error << "WavesAudioBackend::set_device_name (): Failed to initialize device [" << device_name << "]!" << endmsg;
        return -1;
    }
...
WavesAudioBackend::drop_device()
{
	...
	if (_device)
	{
		...
		if (wtErr != eNoErr) {
			PBD::error << "WavesAudioBackend::drop_device (): [" << _device->DeviceName () << "]->SetActive () failed!" << endmsg;
			return -1;
		}
	}
...
WavesAudioBackend::set_sample_rate (float sample_rate)
{
    ...
    if (!_device) {
        PBD::error << "WavesAudioBackend::set_sample_rate (): No device is set!" << endmsg;
        return -1;
    }
    ...
    if (device_needs_restart) {
        ...
        if (retVal != eNoErr) {
            PBD::error << "WavesAudioBackend::set_sample_rate (): [" << _device->DeviceName () << "]->SetStreaming (false) failed (" << retVal << ") !" << endmsg;
            return -1;
        }
    }
    ...
    if (retVal != eNoErr) {
        PBD::error << "WavesAudioBackend::set_sample_rate (): [" << _device->DeviceName() << "]->SetCurrentSamplingRate ((int)" << sample_rate << ") failed (" << retVal << ") !" << endmsg;
        return -1;
    }
    ...
    if (device_needs_restart) {
        ...
        if (retVal != eNoErr) {
            PBD::error << "WavesAudioBackend::set_sample_rate (): [" << _device->DeviceName () << "]->SetStreaming (true) failed (" << retVal << ") !" << endmsg;
            return -1;
        }
    }
    return 0;
...
WavesAudioBackend::set_buffer_size (uint32_t buffer_size)
{
    ...
    if (!_device) {
        PBD::error << "WavesAudioBackend::set_buffer_size (): No device is set!" << endmsg;
        return -1;
    }
    ...
    if (device_needs_restart) {
        ...
        if (retVal != eNoErr) {
            PBD::error << "WavesAudioBackend::set_buffer_size (): [" << _device->DeviceName () << "]->SetStreaming (false) failed (" << retVal << ") !" << endmsg;
            return -1;
        }
    }
    ...
    if (retVal != eNoErr) {
        PBD::error << "WavesAudioBackend::set_buffer_size (): [" << _device->DeviceName() << "]->SetCurrentBufferSize (" << buffer_size << ") failed (" << retVal << ") !" << endmsg;
        return -1;
    }
    ...
    if (device_needs_restart) {
        ...
        retVal  = _device->SetStreaming (true);
        if (retVal != eNoErr) {
            PBD::error << "WavesAudioBackend::set_buffer_size (): [" << _device->DeviceName () << "]->SetStreaming (true) failed (" << retVal << ") !" << endmsg;
            return -1;
        }
    }
...
WavesAudioBackend::sample_rate () const
{
    ...
    if (!_device) {
        PBD::error << "WavesAudioBackend::sample_rate (): No device is set!" << endmsg;
        return -1;
    }
...
WavesAudioBackend::buffer_size () const
{
    ...
    if (!_device) {
        PBD::error << "WavesAudioBackend::buffer_size (): No device is set!" << endmsg;
        return 0;
    }
...
WavesAudioBackend::launch_control_app ()
{
    ...
    if (!_device) {
        PBD::error << "WavesAudioBackend::launch_control_app (): No device is set!" << endmsg;
        return;
    }
    ...
    if (eNoErr != err) {
        PBD::error << "WavesAudioBackend::launch_control_app (): [" << _device->DeviceName () << "]->ShowConfigPanel () failed (" << err << ")!" << endmsg;
    }
...
WavesAudioBackend::_start (bool for_latency_measurement)
{
    ...
    if (!_device) {
        PBD::error << "WavesAudioBackend::_start (): No device is set!" << endmsg;
        stop();
		return -1;
    }

    WTErr retVal = _device->SetActive (true);
    if (retVal != eNoErr) {
        PBD::error << "WavesAudioBackend::_start (): [" << _device->DeviceName () << "]->SetActive () failed!" << endmsg;
		stop();
        return -1;
    }

    if (_register_system_audio_ports () != 0) {
        PBD::error << "WavesAudioBackend::_start (): _register_system_audio_ports () failed!" << endmsg;
        stop();
		return -1;
    }
    ...
    if (_use_midi) {
        if (_midi_device_manager.start () != 0) {
            PBD::error << "WavesAudioBackend::_start (): _midi_device_manager.start () failed!" << endmsg;
            stop();
			return -1;
        }
        if (_register_system_midi_ports () != 0) {
            PBD::error << "WavesAudioBackend::_start (): _register_system_midi_ports () failed!" << endmsg;
            stop();
			return -1;
        }
    }
    ...
    if (engine.reestablish_ports () != 0) {
        PBD::error << "WavesAudioBackend::_start (): engine.reestablish_ports () failed!" << endmsg;
    }
    ...
    retVal  = _device->SetStreaming (true);
    if (retVal != eNoErr) {
        PBD::error << "WavesAudioBackend::_start (): [" << _device->DeviceName () << "]->SetStreaming () failed!" << endmsg;
		stop();
        return -1;
    }
    ...
    if (_use_midi) {
        if (_midi_device_manager.stream (true)) {
            PBD::error << "WavesAudioBackend::_start (): _midi_device_manager.stream (true) failed!" << endmsg;
            stop();
			return -1;
        }
    }
                                           ...
                                           uint64_t cycle_start_time_nanos)
{
    ...
    if ( !engine.thread_initialised_for_audio_processing () ) {
	    PBD::error << "WavesAudioBackend::_audio_device_callback (): It's an attempt to call process callback from the thread which didn't initialize it " << endmsg;
	    
	    AudioEngine::thread_init_callback (this);
    }
    
    if (_main_thread != pthread_self() ) {
            PBD::warning << "WavesAudioBackend::_audio_device_callback (): Process thread ID has changed. Expected thread: " << _main_thread <<  " current thread: " << pthread_self() << std::dec << " !" << endmsg;
            _main_thread = pthread_self();
    }
...
WavesAudioBackend::stop ()
{
	...
	if (_device) {
		...
		// stop device but DO NOT DEACTIVATE IT
		// the device is deactivated in the following specific cases:
		// 1. When restart procedure is required
		// 2. When the device is destroyed (drop_device() function)
		// 
		// Even if symmetrical _start() function DOES activate the device
		// it's done ONLY for the case of the first lauch

		wtErr = _device->SetStreaming (false);
		if (wtErr != eNoErr) {
			PBD::error << "WavesAudioBackend::stop (): [" << _device->DeviceName () << "]->SetStreaming () failed!" << endmsg;
			retVal = -1;
		}
	}
...
WavesAudioBackend::freewheel (bool start_stop)
{
    ...
    if (start_stop != _freewheeling) {
        if (start_stop == true) {
            ...
            if (retval != eNoErr) {
                PBD::error << "WavesAudioBackend::freewheel (): [" << _device->DeviceName () << "]->SetStreaming () failed!" << endmsg;
                return -1;
            }
...
WavesAudioBackend::_freewheel_thread ()
{
    ...
    if (!_freewheel_thread_active) { // Lets create it
        ...
        if (pthread_attr_init (&attributes)) {
            PBD::error << "WavesAudioBackend::freewheel_thread (): pthread_attr_init () failed!" << endmsg;
            return;
        }
   
        if (pthread_attr_setstacksize (&attributes, __thread_stack_size ())) {
            PBD::error << "WavesAudioBackend::freewheel_thread (): pthread_attr_setstacksize () failed!" << endmsg;
            return;
        }
        ...
        if ((pthread_create (&thread_id, &attributes, __start_process_thread, thread_data))) {
            ...
            PBD::error << "WavesAudioBackend::freewheel_thread (): pthread_create () failed!" << endmsg;
            return;
        }
...
WavesAudioBackend::dsp_load () const
{
    ...
    if (!_device) {
        PBD::error << "WavesAudioBackend::cpu_load (): No device is set!" << endmsg;
        return 0;
    }
...
WavesAudioBackend::raw_buffer_size (DataType data_type)
{
    ...
    case DataType::MIDI:
        ...
        default:
            PBD::error << "WavesAudioBackend::raw_buffer_size (): unexpected data type (" << (uint32_t)data_type <<")!" << endmsg;
        break;
    }
    return 0;
}
...
WavesAudioBackend::create_process_thread (boost::function<void ()> func)
{
    ...
    if ((retVal = pthread_attr_init (&attributes))) {
        PBD::error << "Cannot set thread attr init res = " << retVal << endmsg;
        return -1;
    }
   
    if ((retVal = pthread_attr_setstacksize (&attributes, stacksize_aligned))) {
        PBD::error << "Cannot set thread stack size (" << stacksize_aligned << ") res = " << retVal << endmsg;
        return -1;
    }

    if ((retVal = pthread_create (&thread_id, &attributes, __start_process_thread, td))) {
        PBD::error << "Cannot create thread res = " << retVal << endmsg;
        return -1;
    }
...
WavesAudioBackend::join_process_threads ()
{
    ...
    for (std::vector<pthread_t>::const_iterator i = _backend_threads.begin ();
        ...
        if (pthread_join (*i, &status) != 0) {
            PBD::error << "AudioEngine: cannot stop process thread !" << endmsg;
            ret += -1;
        }
