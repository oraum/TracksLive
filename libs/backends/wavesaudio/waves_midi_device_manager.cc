...
WavesMidiDeviceManager::stream (bool yn)
{
    ...
    if (!_active) {
        PBD::error << "WavesMidiDeviceManager::stream (): the midi device manager is not started up !" << endmsg;
        return -1;
    }
    ...
    if (yn)    {
        ...
        if ( Pt_Start (50, __portmidi_callback, this) != ptNoError) {
            PBD::error << "WavesMidiDeviceManager::stream (): Pt_Start () failed!" << endmsg;
            return -1;
        }
    }
    else {
        if (Pt_Stop () != ptNoError) {
            PBD::error << "WavesMidiDeviceManager::stream (): Pt_Stop () failed!" << endmsg;
            return -1;
        }
    }
...
WavesMidiDeviceManager::stop ()
{
    ...
    if (Pm_Terminate () != pmNoError) {
        PBD::error << "WavesMidiDeviceManager::stop (): Pt_Terminate () failed!" << endmsg;
        return -1;
    }
    ...
    for (int i = 0; i < count; i++) {
       ...
       if (strncmp (pm_device_info->name, "Microsoft", strlen ("Microsoft")) == 0) {
        ...
        if (pm_device_info == NULL) {
            PBD::error << "WavesMidiDeviceManager::_create_devices (): Pm_GetDeviceInfo (" << i << ") failed!" << endmsg;
            continue;
        }
        ...
        if (!device) {
			...
			if (device->open (__get_time_ms, (void*)&_audiobackend)) {
				PBD::error << "WavesMidiDeviceManager::_create_devices (): [" << device->name () << "]->open () failed!" << endmsg;
			}
		}
    }
