...
WavesAudioBackend::set_midi_option (const std::string& option)
{
    ...
    if (option == __available_midi_options[1]) {
        ...
        PBD::error << "WavesAudioBackend::set_midi_option (): Invalid MIDI option!" << endmsg;
        return -1;
    }
...
WavesAudioBackend::midi_event_get (pframes_t& timestamp, size_t& size, uint8_t** buffer, void* port_buffer, uint32_t event_index)
{
    ...
    if (buffer == NULL) {
        PBD::error << "WavesAudioBackend::midi_event_get () : NULL in the 'buffer' argument!\n";
        return -1;
    }

    if (port_buffer == NULL) {
        PBD::error << "WavesAudioBackend::midi_event_get () : NULL in the 'port_buffer' argument!\n";
        return -1;
    }
    ...
    if (event_index >= source.size ()) {
        PBD::error << "WavesAudioBackend::midi_event_get () : 'event_index' is out of the number of events stored in 'port_buffer'!\n";
        return -1;
    }
...
WavesAudioBackend::midi_event_put (void* port_buffer, pframes_t timestamp, const uint8_t* buffer, size_t size)
{
    ...
    if (buffer == NULL) {
        PBD::error << "WavesAudioBackend::midi_event_put () : NULL in the 'buffer' argument!\n";
        return -1;
    }

    if (port_buffer == NULL) {
        PBD::error << "WavesAudioBackend::midi_event_put () : NULL in the 'port_buffer' argument!\n";
        return -1;
    }
    ...
    if (target.size () && (pframes_t)target.back ()->timestamp () > timestamp) {
        PBD::error << "WavesAudioBackend::midi_event_put (): The MIDI Event to put is a bit late!" << std::endl;
        PBD::error << "\tprev timestamp is " << (pframes_t)target.back ()->timestamp () << " as the current one is " << timestamp << endmsg;
        return -1;
    }
...
WavesAudioBackend::get_midi_event_count (void* port_buffer)
{
    ...
    if (port_buffer == NULL) {
        PBD::error << "WavesAudioBackend::get_midi_event_count () : NULL in the 'port_buffer' argument!" << endmsg;
        return -1;
    }
...
WavesAudioBackend::midi_clear (void* port_buffer)
{
    ...
    if (port_buffer == NULL) {
        PBD::error << "WavesAudioBackend::midi_clear () : NULL in the 'port_buffer' argument!\n";
        return;
    }
...
WavesAudioBackend::_changed_midi_devices ()
{
    if (_midi_device_manager.stream (false)) {
        PBD::error << "WavesAudioBackend::_changed_midi_devices (): _midi_device_manager.stream (false) failed!" << endmsg;
        return;
    }
    ...
    if (_midi_device_manager.start () != 0) {
        PBD::error << "WavesAudioBackend::_changed_midi_devices (): _midi_device_manager.start () failed!" << endmsg;
        return;
    }

    if (_register_system_midi_ports () != 0) {
        PBD::error << "WavesAudioBackend::_changed_midi_devices (): _register_system_midi_ports () failed!" << endmsg;
        return;
    }
    ...
    if (_midi_device_manager.stream (true)) {
        PBD::error << "WavesAudioBackend::_changed_midi_devices (): _midi_device_manager.stream (true) failed!" << endmsg;
        return;
    }
}
...
WavesAudioBackend::_unregister_system_midi_ports ()
{
    ...
    for (std::vector<WavesMidiPort*>::const_iterator it = physical_midi_ports.begin (); it != physical_midi_ports.end (); ++it) {
        ...
        if (port_iterator == _ports.end ()) {
            PBD::error << "WavesAudioBackend::_unregister_system_midi_ports (): Failed to find port [" << (*it)->name () << "]!"  << endmsg;
        }
        else
    ...
    for (std::vector<WavesMidiDevice*>::const_iterator it = devices.begin (); it != devices.end (); ++it) {
        if ((*it)->is_input ()) {
            ...
            if (midi_port && (midi_port->type () != DataType::MIDI || 
                ...
                PBD::error << "WavesAudioBackend::_register_system_midi_ports (): the port [" << midi_port->name () << "] is inconsystently constructed!" << std::endl;
                disconnect_all (midi_port);
                unregister_port (midi_port);
                port = NULL;
            }
        ...
        if ((*it)->is_output ()) {
            ...
            if (midi_port && (midi_port->type () != DataType::MIDI || 
                ...
                !midi_port->is_physical () ||
                !midi_port->is_terminal ())) {
                PBD::error << "WavesAudioBackend::_register_system_midi_ports (): the port [" << midi_port->name () << "] is inconsystently constructed!" << endmsg;
                disconnect_all (midi_port);
                unregister_port (midi_port);
            }
