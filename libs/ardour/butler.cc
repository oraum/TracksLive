...
void
Butler::update_buffering_parameters ()
{
    Diskstream::set_buffering_parameters (Config->get_buffering_preset());
    
    // Capture ring buffer size MUST BE a MULTIPLE of WRITE_CHUNK size
    framecnt_t write_chunk = Diskstream::disk_write_frames(); // samples
    audio_dstream_capture_buffer_size = (framecnt_t) floor (Config->get_audio_capture_buffer_seconds() * _session.frame_rate()); // samples
    
    audio_dstream_capture_buffer_size += write_chunk - 1;
    audio_dstream_capture_buffer_size -= audio_dstream_capture_buffer_size%write_chunk;
    
    // Capture ring buffer size MUST BE a MULTIPLE of WRITE_CHUNK size
    framecnt_t read_chunk = Diskstream::disk_read_frames(); // samples
    audio_dstream_playback_buffer_size = (framecnt_t) floor (Config->get_audio_playback_buffer_seconds() * _session.frame_rate());
    
    audio_dstream_playback_buffer_size += read_chunk - 1;
    audio_dstream_playback_buffer_size -= audio_dstream_playback_buffer_size%read_chunk;
