...
class LIBARDOUR_API AudioEngine : public SessionHandlePtr, public PortManager
{
  public:
	...
	void           request_backend_reset (bool force = false);
	void           request_device_list_update ();
	void           request_device_connection_loss_handling ();
	...
	*/
	...
	PBD::Signal1<void, std::string> DeviceError;

	/** this signal is emitted about possible sample rate changes from the device side
	    which may require interation with user */
	PBD::Signal0<void> CheckSampleRateMismatch;
  ...
  private:
    ...
    // thread to process hw requests
	Glib::Threads::Thread*     _hw_reset_event_thread;
    // counter for reset requests
	gint                       _hw_reset_request_count;
    // counter for cllbacks that came when reset was ignored
    gint                       _callbacks_after_reset_ignore;
    // flag indicating that reset should be done UNCONDITIONALY
    gint                       _hw_reset_force_reset;
    // condition to suspend hw reset thread when it's in IDLE mode
	Glib::Threads::Cond        _hw_reset_condition;
    // mutex used to sync reset request processing
	Glib::Threads::Mutex       _reset_request_lock;
    // flag that indicates hw reset thread termination
	gint                       _stop_hw_reset_processing;
    
    // thread to process device list updates
	Glib::Threads::Thread*     _hw_devicelist_update_thread;
    // counter for device list update requests
	gint                       _hw_devicelist_update_count;
	// flag indicating that current device has errors
	gint                       _hw_current_device_connection_loss;
    // condition to suspend device list update thread when it's in IDLE mode
	Glib::Threads::Cond        _hw_devicestate_update_condition;
    // mutex used to sync device list update processing
	Glib::Threads::Mutex       _devicestate_update_lock;
    // flag that indicates device list update thread termination
	gint                       _stop_hw_devicelist_processing;

    // watchdog timer to reset or stop current audio device
    // if it does not send AudioCallback for a time which exceeds given timeout
    PBD::WatchdogTimer         _device_watch_dog;
    
	void start_hw_event_processing();
	void stop_hw_event_processing();
	void do_reset_backend();
	void do_device_state_processing();

    void device_callback_timeout ();
