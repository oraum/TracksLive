...
class LIBARDOUR_API Slave {
  public:
    ...
    /**
     * set slave to a sane/insane state
     *
     * @state - state of the slave (chasing just if state == true)
     */
    virtual void set_state (bool state) = 0;
    
    /**
     * reset slave when when slave is stopped (state == false)
     */
    virtual void reset_when_stop () {}
...
class LIBARDOUR_API MTC_Slave : public TimecodeSlave {
  ...
  private:
    ...
    gint state; ///< general state of the MTC (chasing just if state == true)
    bool chasing_now; // true if slave is chasing now
...
public:
  ...
  private:
    ...
    gint state; ///< state of the LTC (chasing just if state == true)
};
