...
class EngineStateController
{
    ...
    /** Set Config's option "latched-record-enable"
     *
     * @param[in] record_locked - value of option "latched-record-enable"
     */
    void                set_latched_record_enable (bool record_locked);
    ...
    /** make sure that current sample_rate is supported and fit session requirements
     * @return true if current sample_rate is valid and is supported, otherwise - false
     */
    bool _validate_sample_rate();
    
    /** make sure that current buffer_size is supported and fit session requirements
     * @return true if current buffer_size is valid and is supported, otherwise - false
     */
    bool _validate_buffer_size();
	...
	*/
	 ...
	 */
	void _on_device_error(std::string);
	/** Invoked when device might change sample rate without a request from Tracks
	 */
	void _on_check_sample_rate_mismatch ();
	/** Invoked when current device changes sample rate after our request
	 */
