...
MTC_Slave::ok() const
{
    return g_atomic_int_get (&state);
}

void
MTC_Slave::set_state (bool yn)
{
    g_atomic_int_compare_and_exchange (&state, !yn, yn);
}

void
MTC_Slave::reset_when_stop()
{
    reset (false);
}
...
{
    if (!g_atomic_int_get (&state)) {
        // port continues to send data even if state == false
        // but no necessity to do something
        return ;
    }
	...
	if (was_full || outside_window (mtc_frame)) {
        ...
        if (chasing_now) {
            ...
            if (session.actively_recording ()) {
                ...
                // before playhead position is changed
                session.request_stop ();
            }
            session.request_locate (mtc_frame, false);
        }
