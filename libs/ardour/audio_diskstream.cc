...
{
    if (playback_buf != 0 && playback_buf->bufsize () == playback_bufsize ) {
        return;
    }
...
{
    if (capture_buf != 0 && capture_buf->bufsize () == capture_bufsize ) {
        return;
    }
...
bool
AudioDiskstream::rename_write_source (const std::string& str) {
    if (write_source_name() == str) {
        return true;
    }
    
    Diskstream::rename_write_source (str);
    /* we need to rename diskstream here
     * because AudioDiskstream::set_name() creates new write_source
     * but we want just to rename a current source
     */
    Diskstream::set_name (str);
    ...
    for (n = 0, i = c->begin(); i != c->end(); ++i, ++n) {
         ...
         if (!recordable()) {
             error << string_compose (_("AudioDiskstream: channel %1 isn't recordable"), n) << endmsg;
             return false;
         }
         ...
         const string new_path = _session.new_audio_source_path (write_source_name(), n_channels().n_audio(), n, destructive (), true);
         ChannelInfo* chan = (*c)[n];
         boost::shared_ptr<AudioFileSource> source = chan->write_source;
