...
Location::Location (const Location& other)
	...
	, _flags (other._flags)
    , _stop_playhead (other._stop_playhead)
...
Location::get_state ()
{
    ...
    node->add_property ("stop-playhead", (_stop_playhead ? "yes" : "no"));
...
Location::set_state (const XMLNode& node, int version)
{
	...
	*/
    ...
    if ((prop = node.property ("stop-playhead")) != 0) {
        _stop_playhead = string_is_affirmative (prop->value());
    } else {
        _stop_playhead = false;
    }
...
void
Location::set_stop_playhead (bool yn)
{
    if (_stop_playhead != yn) {
        _stop_playhead = yn;
        StopChanged ();
    }
}
