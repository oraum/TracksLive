...
void
Session::set_slave_state (bool state)
{
    if (_slave) {
        _slave->set_state (state);
    }
}

bool
Session::get_slave_state () const
{
    return (_slave && _slave->ok ());
}
...
void
Session::start_freewheel ()
{
    if (g_atomic_int_compare_and_exchange (&_freewheel_timeout_enabled, false, true )) {
        bool infinity_freewheel = (Config->get_freewheel_timeout_ms () >= Config->get_max_freewheel_timeout_ms ());
        if (infinity_freewheel == false) {
            Glib::Threads::Mutex::Lock lm (freewheel_lock);
            _freewheel_timeout_function_connection = Glib::signal_timeout ().connect (sigc::mem_fun (*this, &Session::stop_freewheel), Config->get_freewheel_timeout_ms ());
        }
    }
}

gint
Session::get_freewheel_timeout_enabled () const
{
    return g_atomic_int_get (&_freewheel_timeout_enabled);
}

bool
Session::stop_freewheel ()
{
    if (g_atomic_int_compare_and_exchange (&_freewheel_timeout_enabled, true, false)) {
        Glib::Threads::Mutex::Lock lm (freewheel_lock);
        _freewheel_timeout_function_connection.disconnect ();
    }
    return true;
}
...
Session::follow_slave (pframes_t nframes)
{
    ...
    if ( actively_recording ()) {
        // nothing to do - just continue recording
        return true;
    }

	if (!_slave->ok()) {
        // slave cannot (perhaps does not want) follow the master
        _slave->reset_when_stop ();
		stop_transport ();
        stop_freewheel ();
		goto noroll;
	}
	...
	if (!_slave->locked()) {
        if (get_freewheel_timeout_enabled ()) {
            // nothing to do - just continue playback
            return true;
        } else {
    ...
    // have sync connection with master - no need for freewheel
    stop_freewheel ();
...
Session::track_slave_state (float slave_speed, framepos_t slave_transport_frame, framecnt_t /*this_delta*/)
{
	if (slave_speed != 0.0f) {
		...
		if (_transport_speed != 0.0f) {
            if (Config->get_freewheel_timeout_ms () > 0) {
                /* we have to start the freewheel time out. When it will expire we will stop the transport */
                start_freewheel ();
            } else {
                /* we have to stop transport */
                // no freewheel
		...
		if (slave_transport_frame != _transport_frame) {
            if ( actively_recording () ) {
                // we should provide that current record process stopped
                // beforg_timer_starte playhead position is changed
                stop_transport ();
            }
...
Session::process_event (SessionEvent* ev)
{
	...
	*/
	...
	if (non_realtime_work_pending()) {
		...
		/* except events, which we have the capability to handle
         * we also must not remove Skip marker and Stop marker events
         */
        
		if (ev->type != SessionEvent::Locate &&
            ev->type != SessionEvent::Skip &&
            ev->type != SessionEvent::MarkerStop) {
    ...
    case SessionEvent::MarkerStop:
        
        if (actively_recording () == false /*we are not recording*/) {
            if (_stop_marker_sentinel != ev->target_frame /*not transport start position*/) {
                // diasble timecode sync
                set_slave_state (false);
                // stop
                set_transport_speed (0.0, ev->target_frame, false);
            } else {
