...
SndFileSource::write_float (Sample* data, framepos_t frame_pos, framecnt_t cnt)
{
	...
	if (_sndfile != 0) {
		if (last_snd_file_pos != frame_pos) {
			if (sf_seek (_sndfile, frame_pos, SEEK_SET|SFM_WRITE) >= 0) {
				failure = false;
				last_snd_file_pos = frame_pos;
			}
		} else {
			failure = false;
		}
	...
	sf_count_t written_count = sf_writef_float (_sndfile, data, cnt);
	last_snd_file_pos += written_count / _info.channels;
	
	if (written_count != (ssize_t) cnt) {
		return 0;
	}
