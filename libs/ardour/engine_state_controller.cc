...

{
	...
	AudioEngine::instance ()->CheckSampleRateMismatch.connect_same_thread (update_connections, boost::bind (&EngineStateController::_on_check_sample_rate_mismatch, this));
	...
	AudioEngine::instance ()->DeviceError.connect_same_thread (update_connections, boost::bind (&EngineStateController::_on_device_error, this, _1));
...
{
    if (_validate_sample_rate ()) {
        return _validate_buffer_size ();
    }
...
bool
EngineStateController::_validate_sample_rate ()
{
...
bool
EngineStateController::_validate_buffer_size ()
{
    boost::shared_ptr<AudioBackend> backend = AudioEngine::instance ()->current_backend ();
    assert (backend);
...
void
EngineStateController::_on_check_sample_rate_mismatch ()
{
	boost::shared_ptr<AudioBackend> backend = AudioEngine::instance ()->current_backend ();
	assert (backend);
	framecnt_t current_backent_sr = backend->sample_rate ();

	if (_session == NULL) {
		// nothing to check
		_on_sample_rate_change (current_backent_sr); // emit a signal
		return;
	}

	// check session current sample rate
	framecnt_t current_session_sr = _session->frame_rate ();
	if (current_session_sr != current_backent_sr) {
		CheckSampleRateMismatch(); // emit signal
	}
}
...
void
EngineStateController::_on_device_error (std::string what)
{
	set_new_device_as_current ("None");
	push_current_state_to_backend (true);
	DeviceListChanged (false);
...
EngineStateController::push_current_state_to_backend (bool start)
{
	...
	// if session exists - check with session
	if ((state_changed == false) && (_session != NULL)) {
		state_changed = (_current_state->sample_rate != _session->frame_rate() ) ||
			(_current_state->buffer_size != _session->get_block_size());
	}
	...
	if (state_changed) {
		...
		int result = 0;
		{
			PBD::info << "EngineStateController::Setting device: " << _current_state->device_name << endmsg;
			...
			if (!result ) {
				info << "EngineStateController::Setting device sample rate " << _current_state->sample_rate << endmsg;
			...
			if (!result ) {
				info << "EngineStateController::Setting device buffer size " << _current_state->buffer_size << endmsg;
...
void
EngineStateController::set_latched_record_enable (bool record_locked)
{
    Config->set_latched_record_enable (record_locked);
    if (_session && _session->get_record_enabled () && record_locked == false) {
        _session->disable_record (false);
    }
}
