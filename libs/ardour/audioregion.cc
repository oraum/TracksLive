...
AudioRegion::recompute_at_end ()
{
	...
	*/
	...
	if (_left_of_split) {
		...
		if ((_length - _fade_out->back()->when) < _fade_in->back()->when) {
			_fade_out->extend_to (max (64.0, _length - _fade_in->back()->when));
			send_change (PropertyChange (Properties::fade_out));
		}
		if (_fade_out->back()->when > _length) {
	...
	if ((_length - _fade_in->back()->when) < _fade_out->back()->when) {
		_fade_in->extend_to (max (64.0, _length - _fade_out->back()->when));
		send_change (PropertyChange (Properties::fade_out));
	}
	if (_fade_in->back()->when > _length) {
