...
const int32_t Session::default_order_hint = -1;
...
Session::Session (AudioEngine &eng,
	...
	, _suspend_save (0)
	, _save_queued (false)
    , _stop_marker_sentinel (max_framepos)
    ...
    , _ignore_stop_marker_updates (false)
	...
	, _order_hint (default_order_hint)
	, ignore_route_processor_changes (false)
	, _scene_changer (0)
	, _midi_ports (0)
	, _mmc (0)
    , _freewheel_timeout_enabled (0)
{
	uint32_t sr = 0;
	...
	Glib::Threads::RecMutex::Lock lm (AudioEngine::instance()->state_lock());
	...
	if (_is_new ) {
		if ( ARDOUR::Profile->get_trx () ) {
...
#ifdef MAX_NUMBER_OF_MANAGED_PHYSICAL_AUDIO_TRACKS
			// So far Tracks Live MUST NOT give a chance to manage more than 256 tracks.
			if (how_many > MAX_NUMBER_OF_MANAGED_PHYSICAL_AUDIO_TRACKS) {
				how_many = MAX_NUMBER_OF_MANAGED_PHYSICAL_AUDIO_TRACKS;
			}
#endif
			...
			if (Config->get_tracks_auto_naming() == NameAfterDriver) {
				string track_name = "";
				for (std::vector<string>::size_type i = 0; i < how_many; ++i) {
					string track_name;
					track_name = inputs[i];
...
void
Session::update_marks (Location*, bool update_stops)
{
    if (update_stops) {
        Session::sync_locations_to_stops ();
    }
...
Session::sync_locations_to_stops ()
{
    if (_ignore_stop_marker_updates) {
        return;
    }
    
    /* This happens asynchronously (in the audioengine thread). After the clear is done, we will call
     * Session::_sync_locations_to_stops () from the audioengine thread.
     */
    clear_events (SessionEvent::MarkerStop, boost::bind (&Session::_sync_locations_to_stops, this));
}

void
Session::_sync_locations_to_stops ()
{
    /* called as a callback after existing Skip events have been cleared from a realtime audioengine thread */
    ...
    for (Locations::LocationList::const_iterator i = locs.begin(); i != locs.end(); ++i) {
        ...
        if (location && location->is_mark () && location->get_stop_playhead () ) {
            SessionEvent* ev = new SessionEvent (SessionEvent::MarkerStop, SessionEvent::Add, location->start(), location->end(), 0.0);
...
Session::location_added (Location *location)
{
        ...
        if (location->is_mark()) {
                ...
                location->StartChanged.connect_same_thread (mark_update_connections, boost::bind (&Session::update_marks, this, location, true));
                location->EndChanged.connect_same_thread (mark_update_connections, boost::bind (&Session::update_marks, this, location, false));
                location->Changed.connect_same_thread (mark_update_connections, boost::bind (&Session::update_marks, this, location, true));
                location->FlagsChanged.connect_same_thread (mark_update_connections, boost::bind (&Session::update_marks, this, location, true));
                location->StopChanged.connect_same_thread (mark_update_connections, boost::bind (&Session::sync_locations_to_stops, this));
            
                if (location->get_stop_playhead () ) {
                    sync_locations_to_stops ();
                }
        }
...
Session::location_removed (Location *location)
{
        ...
        if (location->is_mark() && location->get_stop_playhead ()) {
            sync_locations_to_stops ();
        }
...
Session::_locations_changed (const Locations::LocationList& locations)
{
	...
	
	{
        ...
        PBD::Unwinder<bool> protect_ignore_stop_mark_updates (_ignore_stop_marker_updates, true);
...
Session::set_frame_rate (framecnt_t frames_per_second)
{
	...
	*/
	...
	if (_base_frame_rate != frames_per_second || _nominal_frame_rate != frames_per_second) {
		...
		// make sure that butler has finished all his work
		if (_butler->does_have_thread()) {
			_butler->summon ();
			_butler->wait_until_finished ();
		}
		reset_write_sources (false);
        _butler->update_buffering_parameters ();
...
void
Session::restore_to_default ()
{
    // remove all multi channel tracks first
    boost::shared_ptr<RouteList> route_list = routes.reader();
    boost::shared_ptr<RouteList> tracks_to_remove (new RouteList);
    {
        RouteList::iterator it = route_list->begin();
        for (; it != route_list->end (); ++it) {
            
            boost::shared_ptr<AudioTrack> track = boost::dynamic_pointer_cast<AudioTrack>(*it);
            
            if (track && track->n_inputs().n_audio() >=2 ) {
                tracks_to_remove->push_back (*it);
            }
        }
    }
    
    const uint32_t n_mono_tracks = this->ntracks () - tracks_to_remove->size ();
    const uint32_t n_inputs = EngineStateController::instance()->get_available_inputs_count();
    
    if (n_inputs < n_mono_tracks) {
        // remove mono tracks amount of which exceeds the number of inputs
        
        uint32_t mono_tracks_to_remove = n_mono_tracks - n_inputs;
        RouteList::reverse_iterator rit = route_list->rbegin();
        while (mono_tracks_to_remove > 0) {
            
            boost::shared_ptr<AudioTrack> track = boost::dynamic_pointer_cast<AudioTrack>(*rit);
            
            if (track && track->n_inputs().n_audio() == 1) {
                tracks_to_remove->push_back(*rit);
                --mono_tracks_to_remove;
            }
    ...
    // remove routes from the session and clear lists to drop shared ptr references
    remove_routes (tracks_to_remove);
    tracks_to_remove->clear ();
    route_list->clear ();
    
    if (n_inputs > n_mono_tracks) {
        // add tracks to cover the lack
        new_audio_track (1, 1, Normal, 0, n_inputs - n_mono_tracks, string() );
    }
}
...
Session::add_routes (RouteList& new_routes, bool input_auto_connect, bool output_auto_connect, bool save)
{
    ...
    // HOT FIX. (REWORK IT)
    // need more investigation while butler is necessary
    if (play_loop) {
        // if loop is active
        // schedule a locate-roll to refill the diskstreams at the
        // previous loop end
...
Session::add_routes_inner (RouteList& new_routes, bool input_auto_connect, bool output_auto_connect)
{
	...
	if (_order_hint > default_order_hint) {
		order = _order_hint;
		_order_hint = default_order_hint;
	}
...
Session::audio_source_name_is_unique (const string& name)
{
	...
	for (vector<string>::const_iterator i = sdirs.begin(); i != sdirs.end(); ++i) {
		...
		*/
        ...
        // check that path has never been used in this session
        if (all_removed_sources_path.find (possible_path) != all_removed_sources_path.end()) {
            existing++;
            break;
        }
	}
...
void
Session::update_meters ()
{
    boost::shared_ptr<RouteList> rl = routes.reader ();
    
    for (RouteList::iterator i = rl->begin(); i != rl->end(); ++i) {
        
        boost::shared_ptr<PeakMeter> meter = (*i)->shared_peak_meter();
        if (meter) {
            meter->publish_meters();
        }
    }
}
