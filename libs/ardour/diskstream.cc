...
bool
Diskstream::rename_write_source (const std::string& str) {
    return Diskstream::set_write_source_name (str);
}
                                   ...
                                   framecnt_t& write_buffer_size)
{
	switch (bp) {
	case Small:
		read_chunk_size = 131072;  /* samples */
#if defined (__APPLE__)
        write_chunk_size = 16384; /* samples */
#elif defined (PLATFORM_WINDOWS)
        write_chunk_size = 32768; /* samples */
#endif
...
#endif
        write_buffer_size = 15; /* seconds */
		read_buffer_size = 10;  /* seconds */
		break;

	case Large:
		read_chunk_size = 524288; /* samples */
		write_chunk_size = 524288; /* samples */
