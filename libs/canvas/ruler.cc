...
Ruler::render (Rect const & area, Cairo::RefPtr<Cairo::Context> cr) const
{
	...
	if (_outline_width == 1.0) {
		...
		cr->move_to (intersection.x0, self.y1-0.5);
		cr->line_to (intersection.x1, self.y1-0.5);
	} else {
		cr->move_to (intersection.x0, self.y1);
		cr->line_to (intersection.x1, self.y1);
	}
	cr->stroke ();
	...
	for (vector<Mark>::const_iterator m = marks.begin(); m != marks.end(); ++m) {
        ...
        if (_divide_height >= 0.0) {
                ...
                cr->move_to (intersection.x0, self.y0 + _divide_height-1.0+0.5);
                cr->line_to (intersection.x1, self.y0 + _divide_height-1.0+0.5);
                cr->stroke ();
